...
setclipboard("https://discord.gg/dSTJBw4sw")
--[[
	WARNING: Heads up! This script has not been verified by ScriptBlox. Use at your own risk!
]]
-- COMMENTED OUT: External script loading removed for stability
...
if not _rfSrc then
warn("[AzuWave] Failed to load Rayfield, check your internet or executor HTTP access.")
return
end
local _loadedFunc, _loadErr = loadstring(_rfSrc)
if not _loadedFunc then
warn("[AzuWave] Rayfield syntax error: " .. tostring(_loadErr))
return
end
local _rfOk, _rfResult = pcall(_loadedFunc)
if not _rfOk then
warn("[AzuWave] Rayfield failed to initialize: " .. tostring(_rfResult))
return
end
Rayfield = _rfResult
local _AzuWaveHttp = game:GetService("HttpService")
local _AzuWave_Elements = {}
local function AzuWave_EnsureFolders()
    pcall(function()
        if not isfolder("AzuWave") then makefolder("AzuWave") end
        if not isfolder("AzuWave/Configs") then makefolder("AzuWave/Configs") end
    end)
end
local function AzuWave_ListConfigs()
    local names = {}
    pcall(function()
        AzuWave_EnsureFolders()
        local files = (listfiles and listfiles("AzuWave/Configs")) or {}
...
local _AzuWave_SkipFlags = { AzuWaveConfigDropdown = true }
local function AzuWave_PackColor(c)
    return {R = math.floor(c.R * 255 + 0.5), G = math.floor(c.G * 255 + 0.5), B = math.floor(c.B * 255 + 0.5)}
end
local function AzuWave_UnpackColor(t)
    return Color3.fromRGB(t.R or 0, t.G or 0, t.B or 0)
end
local function AzuWave_SaveConfig(name)
    AzuWave_EnsureFolders()
    local data = {}
    for flag, obj in pairs(Rayfield.Flags) do
        if not _AzuWave_SkipFlags[flag] then
            pcall(function()
                if obj.Type == "ColorPicker" then
                    if obj.Color then
                        data[flag] = AzuWave_PackColor(obj.Color)
                    end
    ...
    local ok, json = pcall(function() return _AzuWaveHttp:JSONEncode(data) end)
    if ok then
        pcall(function() writefile("AzuWave/Configs/" .. name .. ".json", json) end)
    end
end
local function AzuWave_LoadConfig(name)
    local path = "AzuWave/Configs/" .. name .. ".json"
    local exists = false
    pcall(function() exists = isfile(path) end)
    if not exists then return false end
    local ok, data = pcall(function()
        return _AzuWaveHttp:JSONDecode(readfile(path))
    end)
    if ok and data then
                ...
                task.spawn(function()
                    pcall(function()
                        ...
                        if flagObj and flagObj.Type == "ColorPicker" and type(v) == "table" then
                            v = AzuWave_UnpackColor(v)
                        end
                        local el = _AzuWave_Elements[f]
...
local function AzuWave_PatchTab(tab)
...
local queueTeleport = queue_on_teleport or (syn and syn.queue_on_teleport) or (fluxus and fluxus.queue_on_teleport) or nil
...
local function saveTheme(theme)
    pcall(function()
        if not isfolder("AzuWave") then makefolder("AzuWave") end
        writefile("AzuWave/Theme.txt", theme)
    end)
end
local function loadTheme()
    local theme = nil
    pcall(function()
        if isfile("AzuWave/Theme.txt") then
            local t = readfile("AzuWave/Theme.txt"):match("^%s*(.-)%s*$")
...
do
    ...
    function dolaghighjump(skipChecks)
        ...
        if hrp and hum then
            ...
            task.wait(0.01)
            if _lhj_freeze and _lhj_freeze > 0 then
                task.wait(_lhj_freeze / 1000)
            end
    ...
    -- Expose toggle so Admin tab UI can enable/disable
    _G._AzuWave_SetLedge = function(v)
        _LedgeEnabled    = v
        _FpsBounceEnabled = v
    end
...
if isLobby then
    local _lobbyTheme = loadTheme() or "Default"
    local Window = Rayfield:CreateWindow({
        Name = "AzuWave | discord.gg/bq2ah37FX",
        LoadingTitle = "AzuWave (Lobby)",
        LoadingSubtitle = "made by kobi and irib",
        Icon = "flame",
        ShowText = "AzuWave",
        Theme = _lobbyTheme,
...
_utab:CreateSlider({
    ...
    Flag = "WalkSpeed",
    Callback = function(Value)
        workspace[player.Name].Humanoid.WalkSpeed = Value
    end,
})
_utab:CreateSlider({
    Name = "Jump Power",
    ...
    Flag = "JumpPower",
    Callback = function(Value)
        workspace[player.Name].Humanoid.JumpPower = Value
    end,
})
_utab:CreateSlider({
    Name = "Field of View",
...
_utab:CreateButton({
    ...
    Callback = function()
        local nlist = {}
        for _, v in ipairs(game.ReplicatedStorage:WaitForChild("Season"):WaitForChild("Players"):GetChildren()) do
...
_utab:CreateButton({
    ...
    Callback = function()
        for i, v in ipairs(game.ReplicatedStorage.Season.Players:GetChildren()) do
...
local function removeHighlight_U(player)
   if playerHighlights_U[player] then
      playerHighlights_U[player]:Destroy()
      playerHighlights_U[player] = nil
   end
end

local function applyHighlight_U(player)
   if not Config.highlightEnabled or player == LocalPlayer_U then return end
   removeHighlight_U(player)
   ...
   if character then
      ...
      h.Parent = character
      playerHighlights_U[player] = h
   end
end

local function removeHitboxExpansion(player)
...
local function applyHitboxExpansion(player)
   if not Config.hitboxEnabled or player == LocalPlayer_U then return end
...
local function onPlayerAdded_U(player)
   playerConnections_U[player] = {}
   playerConnections_U[player].CharacterAdded = player.CharacterAdded:Connect(function()
      task.wait(0.1)
      applyHighlight_U(player)
...
local function onPlayerRemoving_U(player)
   removeHighlight_U(player)
   removeHitboxExpansion(player)
   if playerConnections_U[player] then
      for _, conn in pairs(playerConnections_U[player]) do
         conn:Disconnect()
      end
      playerConnections_U[player] = nil
   end
end
...
local function getClosestTarget_U()
   ...
   for player, _ in pairs(playerConnections_U) do
      if player == LocalPlayer_U then continue end
      local char = player.Character
...
for _, player in ipairs(Players_U:GetPlayers()) do
   if player ~= LocalPlayer_U then onPlayerAdded_U(player) end
end
...
if isPractice then
    local Window = Rayfield:CreateWindow({
        Name = "AzuWave | discord.gg/bq2ah37FX",
        LoadingTitle = "AzuWave (Practice)",
        LoadingSubtitle = "made by kobi and irib",
        Icon = "flame",
        ShowText = "AzuWave",
        Theme = loadTheme() or "Default",
        DisableRayfieldPrompts = false,
        DisableBuildWarnings = false,
        MinimizeKey = Enum.KeyCode.LeftControl,
        CustomUIStrings = {
...
local Window = Rayfield:CreateWindow({
    Name = "AzuWave | discord.gg/bq2ah37FX",
    LoadingTitle = "AzuWave (" .. (isCamp and "Camp" or isMovies and "Movies" or isExpedition and "Expedition" or "Unknown") .. ")",
    LoadingSubtitle = "made by kobi and irib",
    Icon = "flame",
    ShowText = "AzuWave",
    Theme = loadTheme() or "Default",
...
local function equipGear(toolName)
    ...
    if not _gearFolder then warn("[AzuWave] Gear folder not found!") return end
    local _gearTool = _gearFolder:FindFirstChild(toolName)
    if not _gearTool then warn("[AzuWave] Tool not found: " .. toolName) return end
    ...
    Callback = function(Value)
        ...
        local function getPlayerTeamColor(player)
            local playerData = ReplicatedStorage:FindFirstChild("Season")
                               ...
                               and ReplicatedStorage.Season.Players:FindFirstChild(player.Name)
        ...
        local function applyNameTag(player)
            ...
            text.TextColor3 = getPlayerTeamColor(player)
...
if isCamp then
    ...
    local elimFolder = ws:WaitForChild("ElimMethod")
    local sleighTemplate = rs.Products.ElimMethods.Camp["Sleigh Elimination"]
    ...
    local function isEliminationModel(v)
        if not v:IsA("Model") then return false end
        local n = v.Name:lower()
        return (
            n:find("boat") or n:find("toilet") or n:find("cart")
            or n:find("rocket") or n:find("cannon") or n:find("elimination")
        )
    end

    local function playEffects(clone)
...
ToolsTab:CreateButton({
    ...
    Callback = function()
        ...
        -- Watch a single player's wins value and trigger reanimate when it goes up
...
SettingsTab:CreateButton({
    ...
    Callback = function()
        ...
        local path = "AzuWave/Configs/" .. _dwSelectedConfig .. ".json"
        local deleted = false
        pcall(function()
...
MainTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            voteConn = game.ReplicatedStorage.Season.Voting.Votes.ChildAdded:Connect(function(v)
                local r = game.ReplicatedStorage.Season.Players[v.Value].Value
                local d = game.ReplicatedStorage.Season.Players[v.Name].Value
...
MainTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            printVotesConn = game.ReplicatedStorage.Season.Voting.Votes.ChildAdded:Connect(function(v)
                local r = game.ReplicatedStorage.Season.Players[v.Value].Value
                local d = game.ReplicatedStorage.Season.Players[v.Name].Value
                print(r .. " voted for " .. d)
            end)
...
MainTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            exposeVotesConn = game.ReplicatedStorage.Season.Voting.Votes.ChildAdded:Connect(function(v)
                local r = game.ReplicatedStorage.Season.Players[v.Value].Value
                local d = game.ReplicatedStorage.Season.Players[v.Name].Value
                game.TextChatService.TextChannels.RBXGeneral:SendAsync(r .. " voted for " .. d)
            end)
        else
        ...
        if Value then
            ...
            local function watchJuror(j)
                ...
                local conn = l.ChildAdded:Connect(function(v)
                    local voter = j.Value
                    local votee = v.Value
                    if game.ReplicatedStorage.Season.Players:FindFirstChild(v.Name) then
                        votee = game.ReplicatedStorage.Season.Players[v.Name].Value
                    end
                    Rayfield:Notify({ Title = "Jury Vote", Content = voter .. " voted for " .. votee, Duration = 3, Image = "bell-ring" })
                end)
                table.insert(juryVoteConns, conn)
            end
            for _, j in pairs(game.ReplicatedStorage.Season.Jury:GetChildren()) do
                watchJuror(j)
            end
            local addConn = game.ReplicatedStorage.Season.Jury.ChildAdded:Connect(watchJuror)
...
if isCamp then
    MainTab:CreateToggle({
        ...
        Callback = function(Value)
            if Value then
                exileVotesConn = game.ReplicatedStorage.Season.Twists.ExileVoting.Votes.ChildAdded:Connect(function(v)
                    local r = game.ReplicatedStorage.Season.Players[v.Value].Value
                    local d = game.ReplicatedStorage.Season.Players[v.Name].Value
...
MainTab:CreateToggle({
    ...
    Callback = function(v)
        ...
        if v then
            local RS = game:GetService("ReplicatedStorage")
            pcall(function()
                _whVC = RS.Season.Voting.Votes.ChildAdded:Connect(function(vote)
                    pcall(function()
                        sendWH(RS.Season.Players[vote.Value].Value .. " voted for " .. RS.Season.Players[vote.Name].Value)
                    end)
                end)
            end)
        end
...
local selectedArtChar = "Swastika"
CharactersTab:CreateDropdown({
    Name = "ASCII Art Character Names",
    Options = { "Swastika", "Skull", "Guy", "Hacker", "Middle Finger" },
...
do
    ...
    -- ── Build skin list from CharacterSelection ───────────────────────────────
    ...
    CharactersTab:CreateButton({
        ...
        Callback = function()
            ...
            for _, _category in ipairs(_rs.Products.Shop.Items:GetChildren()) do
                for _, _item in ipairs(_DataStore[_category.Name]:GetChildren()) do
                    _item:Destroy()
                end
                for _, _item in ipairs(_category:GetChildren()) do
                    _item:Clone().Parent = _DataStore[_category.Name]
                end
            end
        end,
    })
    ...
    -- Collect unique parts across all skins
    local _csb_faceOptions    = {"None"}
    local _csb_shirtOptions   = {"None"}
    local _csb_pantsOptions   = {"None"}
    local _csb_accOptions     = {"None"}

    local _csb_faceMap  = {}  -- label -> texture string
    local _csb_shirtMap = {}  -- label -> Shirt instance
    local _csb_pantsMap = {}  -- label -> Pants instance
    local _csb_accMap   = {}  -- label -> Accessory/Hat instance
    ...
    -- Move "None" back to front after sort
    ...
    -- Helper: apply a single accessory to character
    ...
    -- ── Save / Load custom skin ───────────────────────────────────────────────
    -- Saves current char's shirt/pants/face/accessories as a named combo to file
    local function _csb_getSavedSkins()
        local names = {"None"}
        pcall(function()
            if not isfolder("AzuWave") then makefolder("AzuWave") end
            if not isfolder("AzuWave/CustomSkins") then makefolder("AzuWave/CustomSkins") end
            local files = listfiles and listfiles("AzuWave/CustomSkins") or {}
            for _, path in ipairs(files) do
                local name = tostring(path):match("([^/\]+)%.json$")
                if name and name ~= "" then table.insert(names, name) end
            end
    ...
    CharactersTab:CreateInput({
        ...
        Callback = function(text)
            ...
            pcall(function()
                if not isfolder("AzuWave") then makefolder("AzuWave") end
                if not isfolder("AzuWave/CustomSkins") then makefolder("AzuWave/CustomSkins") end
                writefile("AzuWave/CustomSkins/" .. saveName .. ".json", game:GetService("HttpService"):JSONEncode(data))
            end)
    ...
    _csb_loadDropdown = CharactersTab:CreateDropdown({
        ...
        Callback = function(Option)
            ...
            local path = "AzuWave/CustomSkins/" .. name .. ".json"
            ...
            local char = _SC_LocalPlayer.Character
            if not char then return end
            -- Clear existing clothes/accessories
            for _, v in ipairs(char:GetChildren()) do
            ...
            -- Accessories (re-apply from skin data by name)
    ...
    CharactersTab:CreateButton({
        ...
        Callback = function()
            ...
            local path = "AzuWave/CustomSkins/" .. _csb_selectedLoad .. ".json"
            local deleted = false
            pcall(function()
                if isfile(path) then
                    delfile(path)
...
-- Rainbow Marshmallow | Tints equipped marshmallow with rainbow colours | by kobi
do
    ...
    ClientTab:CreateToggle({
        ...
        Callback = function(Value)
            ...
            if Value then
                ...
                -- Reset the marshmallow tint back to white (default)
                local imageLabel = _getImageLabel()
                if imageLabel then
...
ClientTab:CreateInput({
    ...
    Flag = "CustomCoins",
    Callback = function(Text)
        game.Players.LocalPlayer.DataStore.Coins.Value = Text
    end,
})
ClientTab:CreateInput({
    ...
    Flag = "CustomCampWins",
    Callback = function(Text)
        game.Players.LocalPlayer.DataStore.CampWins.Value = Text
    end,
})
ClientTab:CreateInput({
    ...
    Callback = function(Text)
        game.Players.LocalPlayer.DataStore.MoviesWins.Value = Text
    end,
})
ClientTab:CreateInput({
    ...
    Callback = function(Text)
        game.Players.LocalPlayer.DataStore.ExpeditionWins.Value = Text
    end,
})
ClientTab:CreateInput({
    ...
    Callback = function(Text)
        game.Players.LocalPlayer.DataStore.OfficialWins.Value = Text
    end,
})
ClientTab:CreateButton({
    ...
    Callback = function()
        ...
        local player = Players.LocalPlayer
        local avatarUrl = "https://www.roblox.com/headshot-thumbnail/image?userId=" .. player.UserId .. "&width=48&height=48&format=png"
        local scrollFrame = workspace.WinLeaderboard.SurfaceGui.ScrollingFrame
        for _, frame in pairs(scrollFrame:GetChildren()) do
            if frame:IsA("Frame") then
                local placeLabel = frame:FindFirstChild("Image") and frame.Image:FindFirstChild("Place")
                if placeLabel and placeLabel.Text == "1" then
...
ChallengesTab:CreateButton({
    ...
    Callback = function()
        local m = workspace.Assets:FindFirstChild("Monster"); if m then local n = m:FindFirstChild("MonsterNPC"); if n then n:Destroy() end end
    end,
})
ChallengesTab:CreateButton({
    Name = "Collect Egg",
    Callback = function()
        local RS = game:GetService("ReplicatedStorage")
        local alien = workspace.Assets:WaitForChild("Alien"); if not alien then return end
        local char = RS.Season.Players:FindFirstChild(player.Name); local cv = char and char.Value; if not cv then return end
        for _, v in pairs(alien:GetDescendants()) do
            if v.ClassName == "TextLabel" and v.Text == cv then
                local root = v.Parent.Parent.Parent; root.CFrame = player.Character.HumanoidRootPart.CFrame; root.CanCollide = false
            end
        end
    end,
})
...
ChallengesTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            ...
            if cf then _ancientConn = cf.ChildAdded:Connect(function(coin)
                if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                    coin.Position = player.Character.HumanoidRootPart.Position; coin.Transparency = 0; coin.CanCollide = false
                end
            end) end
        else
...
ChallengesTab:CreateToggle({
    ...
    Callback = function(Value)
        _expObby = Value
        if Value then task.spawn(function() while _expObby do
            local f = workspace.Assets:FindFirstChild("Finish", true)
            if f and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                f.CanCollide = false; f.Transparency = 1; f.Position = player.Character.HumanoidRootPart.Position
            end; task.wait(0.5)
...
ChallengesTab:CreateButton({
    ...
    Callback = function()
        for _, v in pairs(workspace.Assets:GetDescendants()) do
            if v.Name == "SpleefPart" then firetouchinterest(player.Character.HumanoidRootPart, v, 0) end
        end
    end,
})
...
ChallengesTab:CreateToggle({
    ...
    Callback = function(Value)
        _clovers = Value
        task.spawn(function() while _clovers do task.wait(0.1)
            for _, obj in pairs(workspace:GetDescendants()) do
                if obj.Name == "Gem" or obj.Name == "Coin" then obj.Transparency = 1; obj.Position = player.Character.Torso.Position end
            end
        end end)
    end,
})
...
ChallengesTab:CreateToggle({
    ...
    Callback = function(Value)
        _rings = Value
        task.spawn(function() while _rings do task.wait(0.1)
            for _, obj in pairs(workspace:GetDescendants()) do
                if obj.Name == "RingHitbox" then obj.Transparency = 1; obj.Position = player.Character.Torso.Position end
            end
        end end)
    end,
})
            ...
            task.spawn(function()
                while _hawaiiDigAll do
                    ...
                    if Assets and Assets:FindFirstChild("Hawaii") then
                        local ReplicatedStorage = game:GetService("ReplicatedStorage")
                        for _, v in ipairs(ReplicatedStorage.Seasons.Teams:GetChildren()) do
...
-- Recorder state (Movement Recorder in Autoplay tab)
...
local function _rec_getFiles()
    ...
    pcall(function()
        if not isfolder("AzuWave") then makefolder("AzuWave") end
        if not isfolder("AzuWave/Recordings") then makefolder("AzuWave/Recordings") end
        local files = (listfiles and listfiles("AzuWave/Recordings")) or {}
        for _, path in ipairs(files) do
...
local function TAS_LoadFromSource(sourceInfo)
    ...
    elseif type(sourceInfo) == "table" then
        ...
        elseif sourceInfo.type == "direct" then return sourceInfo  -- pass through, handled separately
...
local function TAS_RunAutoPlayWithPathfind()
    ...
    -- Direct type: self-contained script (e.g. wallhop), just run it once walkspeed is up
...
end -- if not isMovies (placement tracker)
...
if isCamp then
    ...
    TAS_SelectedCaveChaos = "Cave Chaos"
    AutoplayTab:CreateDropdown({
    ...
    TAS_SelectedConstructCourse = "Construct Course"
    AutoplayTab:CreateDropdown({
    ...
    TAS_SelectedColosseumClimb = "Colosseum Climb"
    -- Sword Fight Autoplay (Camp)
...
AutoplayTab:CreateToggle({
    ...
    Callback = function(Value)
        ...
        if not Value then
            -- If toggled off mid-recording, stop the recorder
            if _rec_isRec then
                _rec_isRec = false
        ...
        task.spawn(function()
            while _rec_autoRoundRecord do
                if TAS_IsInChallenge() then
                    local Hum = TAS_LocalPlayer.Character
                              and TAS_LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
                    -- Detect round freeze (WalkSpeed == 0)
                    if Hum and Hum.WalkSpeed == 0 then
                        -- Wait for round to actually start (WalkSpeed goes back up)
                        while _rec_autoRoundRecord and TAS_IsInChallenge() do
                            Hum = TAS_LocalPlayer.Character
                                  and TAS_LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
                            if Hum and Hum.WalkSpeed >= 16 then break end
                            task.wait(0.05)
                        end
                        if not _rec_autoRoundRecord or not TAS_IsInChallenge() then
                            ...
                            -- Stop recording when player touches any part named "Finish"
                            ...
                            _rec_finishConn = {}
                            connectFinishParts()
                            -- Wait until recording stops (finish touched) or toggle turned off
                            while _rec_autoRoundRecord and _rec_isRec do
                                task.wait(0.2)
                            end
                            -- Clean up any leftover finish connections
                            if _rec_finishConn then
                                for _, fc in ipairs(_rec_finishConn) do fc:Disconnect() end
                                _rec_finishConn = nil
                            end
                            -- Also wait for challenge to fully end before next loop
                            while _rec_autoRoundRecord and TAS_IsInChallenge() do
                                task.wait(0.5)
                            end
                        end
...
-- Save Recording (type name + press Enter)
AutoplayTab:CreateInput({
    ...
    Callback = function(text)
        ...
        local name     = (_rec_fileName and _rec_fileName ~= "") and _rec_fileName
                         or ("Recording_" .. os.time())
        local filePath = "AzuWave/Recordings/" .. name .. ".lua"

        -- Build the standalone replay script
        local c = "-- AzuWave Movement Recording (world coords)\n"
        ...
        pcall(function() writefile(filePath, c) end)
        if _rec_statusElem then _rec_statusElem:Set("💾 Saved: " .. name) end
        end
    end,
})

-- Load dropdown — populated from AzuWave/Recordings/ at script load
...
-- Play Selected on Round Start toggle
    ...
    Callback = function(Value)
        ...
        task.spawn(function()
            while _rec_autoRoundPlay do
                -- Wait until we're in a challenge
                if TAS_IsInChallenge() then
                    local Hum = TAS_LocalPlayer.Character
                              and TAS_LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
                    -- Detect round freeze (WalkSpeed == 0)
                    if Hum and Hum.WalkSpeed == 0 then
                        ...
                        if not _rec_autoRoundPlay or not TAS_IsInChallenge() then
                            ...
                            if _rec_selectedLoad
                               and _rec_selectedLoad ~= ""
                               and _rec_selectedLoad ~= "(no recordings)"
                               and _rec_selectedLoad ~= "(nothing)" then
                                local filePath = "AzuWave/Recordings/" .. _rec_selectedLoad .. ".lua"
                                local ok, src = pcall(readfile, filePath)
                                if ok and src and src ~= "" then
                                    local fn, err = loadstring(src)
                                    if fn then
                                        if _rec_statusElem then
                                            _rec_statusElem:Set("▶ Auto: " .. _rec_selectedLoad)
                                        end
                                        task.spawn(fn)
                                    else
                                        if _rec_statusElem then
                                            _rec_statusElem:Set(" Error: " .. tostring(err))
                                        end
                                    end
                                else
                                    if _rec_statusElem then
                                        _rec_statusElem:Set(" File not found!")
                                    end
                                end
                            end
                            -- Wait until this challenge ends before listening for next round
                            while _rec_autoRoundPlay and TAS_IsInChallenge() do
                                task.wait(0.5)
                            end
                        end
...
AutoplayTab:CreateDivider()
    Rayfield:Notify({
        Title = "azu wave, better than dramaware (now fully AzuWave).",
        Content = "azu wave is now up!\nDiscord copied to clipboard.",
        Duration = 6,
    })
